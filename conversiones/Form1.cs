...
namespace conversiones
{
    public partial class Form1 : Form
    {
        ...
        private void btnconversor_Click(object sender, EventArgs e)
        {
            try
            {
                lblResultado.Text = "Resultado: " + conversor.convertir(cbotengo.SelectedIndex, cboquiero.SelectedIndex, double.Parse(txtcantidades.Text), cbotipos.SelectedIndex)+""+conversor.Etiquetas[cbotipos.SelectedIndex][cboquiero.SelectedIndex];
            }
            catch (Exception error)
            {
                MessageBox.Show("Error en Datos", "conversor", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            }
